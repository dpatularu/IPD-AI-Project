...
class GenPlayer:
    ...
    def grudger(memDepth: int) -> Player:
        """ Creates a player that initially cooperates, but will always
            defect if the opponent defects once. """
        n: int = Player.calcStratSize(memDepth) - 1
    ...
    def random_list(memDepth: int, n: int) -> List[Player]:
        ...
        return [GenPlayer.random(memDepth) for _ in range(n)]
    ...
    def all_handpicked(memDepth: int) -> List[Player]:
        """Returns a list of all handpicked strategies. """
        return [
...
class GenDna:
    ...
    def randomLst(n: int, memDepth: int, size: int = None) -> List[Dna]:
        ...
        return [GenDna.random(memDepth, size) for _ in range(n)]
    ...
    def pavlov() -> Dna:
        """ Cooperates if it and its opponent moved alike in previous move
            and defects if they moved differently. Cooperates on first move. """
        return Dna("CDDCC")

    @staticmethod
    def susPavlov() -> Dna:
        """ Cooperates if it and its opponent moved alike in previous move
            and defects if they moved differently. Defects on first move. """
        return Dna("CDDCD")
